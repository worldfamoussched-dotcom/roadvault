...
const frameDir = path.join(outDir, "founder-kit-frames");
const videoPath = path.join(outDir, "roadvault-founder-kit-webgl.mp4");
...
async function main() {
  ...
  await page.goto(`http://127.0.0.1:${port}/presentation/animation/founder-kit-cinematic.html?capture=1`, {
    waitUntil: "networkidle",
  });
  await page.waitForFunction(() => window.__roadVaultReady === true);

  const stills = [
    { name: "founder-kit-pouch-prototype.png", seconds: 0.25 },
    { name: "founder-kit-floating-components.png", seconds: 2.8 },
    { name: "founder-kit-xray-hidden-pocket.png", seconds: 6.5 },
  ];
